        ...
        ILogger<CancellationService> logger) : ICancellationService
    {
        ...
        public async Task<ApiResponse<CancellationResponse>> RequestCancellationAsync(CancellationRequest cancellationRequest)
        {
            try
            {
                ...
                if (order == null)
                {
                    return new ApiResponse<CancellationResponse>(404, "Order not found.");
                }

                // 2. Check if order is eligible for cancellation (only Processing)
                if (order.OrderStatus != OrderStatus.Processing)
                {
                    return new ApiResponse<CancellationResponse>(400, "Order is not eligible for cancellation.");
                }
                ...
                {
                    return new ApiResponse<CancellationResponse>(400, "A cancellation request for this order already exists.");
                }
                ...
                unitOfWork.CancellationRepository.Add(cancellation);
                
                // 5. Save changes to DB
                await unitOfWork.SaveChangesAsync();

                // 6. Manual mapping from Cancellation to CancellationResponse
                ...
                return new ApiResponse<CancellationResponse>(200, cancellationResponse);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ApiResponse<CancellationResponse>> GetCancellationByIdAsync(int id)
        {
            try
            {
                ...
                {
                    return new ApiResponse<CancellationResponse>(404, "Cancellation request not found.");
                }
        ...
        public async Task<ApiResponse<ConfirmationResponse>> UpdateCancellationStatusAsync(CancellationStatusUpdateRequest statusUpdate)
        {
            ...
            try
            {
                ...
                if (cancellation == null)
                {
                    return new ApiResponse<ConfirmationResponse>(404, "Cancellation request not found.");
                }

                if (cancellation.Status != CancellationStatus.Pending)
                {
                    return new ApiResponse<ConfirmationResponse>(400, "Only pending cancellation requests can be updated.");
                }
                ...
                if (statusUpdate.Status == CancellationStatus.Approved)
                {
                    ...
                    foreach (var item in cancellation.Order.OrderItems)
                    {
        ...
        public async Task<ApiResponse<List<CancellationResponse>>> GetAllCancellationsAsync()
        {
            try
            {
                var cancellations = await unitOfWork.CancellationRepository.GetAllWithOrderAsync(trackChanges: false);

                var cancellationList = cancellations.Select(c => new CancellationResponse
                {
                    Id = c.Id,
                    OrderId = c.OrderId,
                    Reason = c.Reason,
        ...
        private async Task NotifyCancellationAcceptedAsync(Cancellation cancellation)
        {
            ...
            await emailService.SendEmailAsync(cancellation.Order.Customer.Email, subject, emailBody, isBodyHtml: true);
        }
